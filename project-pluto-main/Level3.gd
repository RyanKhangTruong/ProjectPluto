...
var total_triple = 0 
var defeated = 0
const max_enemy_A = (-1 if testing else 16) # 16
const max_enemy_B = (-1 if testing else 5) # 5
const max_enemy_C = (-1 if testing else 4) # 4
...
onready var FINALBOSS = preload("res://FinalBoss.tscn")
...
func _on_SpawnTimer_timeout():
	...
	if defeated == max_enemies:
		$SpawnTimer.stop()
		var finalboss = FINALBOSS.instance()
		add_child(finalboss)
		finalboss.position = $FinalBossSpawn.position
		
func next_level():
	$"PauseButton/PauseButton".hide()
	$"UnpauseButton/UnpauseButton".hide()
	ok = get_tree().change_scene("res://win.tscn")
	$"UnpauseButton/UnpauseButton".hide()
	$PauseButton.hide()
	if get_tree().change_scene("res://win.tscn") != ok:
